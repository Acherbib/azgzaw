...
import com.azgzaw.app.config.KafkaProperties;
...
class AzgzawKafkaResourceIT {
    ...
    void setup() {
        ...
        AzgzawKafkaResource kafkaResource = new AzgzawKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/azgzaw-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/azgzaw-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
